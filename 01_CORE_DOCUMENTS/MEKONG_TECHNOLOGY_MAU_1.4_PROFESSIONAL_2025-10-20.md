...
Dự án đặt mục tiêu **35% doanh thu từ xuất khẩu ASEAN** (stretch goal) vào 2035, tương đương **36,23 triệu USD**. **Base Case:** 25% xuất khẩu ASEAN = 25,88 triệu USD. Đóng góp quan trọng vào thành công tổng thể của dự án.
...
| **R&D** | 36 người, 2,15 triệu USD/năm | 2-5 người, 0,05-0,2 triệu USD/năm | Gấp 10-40 lần |
...
Với chiến lược lấp đầy khoảng trống này, Mekong Technology có thể chiếm **8% thị phần IoT** (stretch goal) và **5% thị phần Robot** (stretch goal) tại Việt Nam, tương đương doanh thu **39,86 triệu USD** từ thị trường nội địa. **Base Case:** 5% thị phần IoT + 3% thị phần Robot = 24,91 triệu USD doanh thu.
...
```mermaid
gantt
    title Cửa sổ cơ hội để tham gia thị trường
    dateFormat  YYYY-MM
    section Mekong Technology
    Khởi động dự án       :done, 2025-01, 2025-06
    Phát triển sản phẩm   :active, 2025-07, 2026-12
    Ra mắt sản phẩm       :crit, 2027-01, 2027-06
    Chiếm thị phần        :2027-07, 2029-12
    section Đối thủ
    Siemens R&D center    :crit, 2025-06, 2026-06
    Schneider mở rộng     :crit, 2025-03, 2025-12
    Startup huy động vốn  :crit, 2024-12, 2026-06
    section Ngưỡng quan trọng
    Ngưỡng cạnh tranh tăng :milestone, 2027-01, 0d
```
...
graph TB
    subgraph Legend
        L1[RPN 16-20: Rất cao]
        L2[RPN 12-15: Cao]
        L3[RPN 9-11: Trung bình]
        L4[RPN 1-8: Thấp]
    end
    
    subgraph Risk_Matrix[Ma trận 4x5]
        R1_pos[R1: Chính sách - RPN 12]
        R2_pos[R2: COVID - RPN 12]
        R3_pos[R3: Chiến tranh TM - RPN 16]
        R4_pos[R4: IP lawsuit - RPN 18]
        R5_pos[R5: Lãi suất - RPN 12]
        R6_pos[R6: Tỷ giá - RPN 9]
        R7_pos[R7: Dumping TQ - RPN 16]
        R8_pos[R8: Chậm TT - RPN 9]
    end
    
    style R4_pos fill:#ff6b6b,stroke:#c92a2a,stroke-width:3px
    style R3_pos fill:#ff6b6b,stroke:#c92a2a,stroke-width:3px
    style R7_pos fill:#ff6b6b,stroke:#c92a2a,stroke-width:3px
    style R1_pos fill:#ffa94d,stroke:#f76707,stroke-width:2px
    style R2_pos fill:#ffa94d,stroke:#f76707,stroke-width:2px
    style R5_pos fill:#ffa94d,stroke:#f76707,stroke-width:2px
    style R6_pos fill:#ffe066,stroke:#fab005,stroke-width:1px
    style R8_pos fill:#ffe066,stroke:#fab005,stroke-width:1px
```
...
|---|---|---:|---|---|
| **Outsource 30% R&D** | Công ty Ấn Độ/Đông Âu (Luxoft, EPAM, ScienceSoft) | 40K USD/person/năm | +3 tháng | ✅ Save 40%<br/>❌ Coordination overhead |
| **Partnership ĐH** | Bách Khoa HCM/HN: Part-time researchers + grad students | 15K USD/person/năm | +6 tháng | ✅ Save 75%<br/>❌ Quality risk |
| **Tăng lương 30%** | 60K → 78K USD/năm để compete | +648K USD/năm | No delay | ✅ Attract talent<br/>❌ Burn rate cao |
| **Remote international** | Hire developers từ VN overseas (US, EU) | 50K USD/person/năm | +1 tháng | ✅ Talent pool lớn<br/>❌ Timezone |
...
|---|---|---:|---|---|
| **Option A: Giảm quy mô 25%** | - Delay OHT Lab (5 engineers)<br/>- Giảm marketing 50%<br/>- Outsource R&D 30% | -5M | No new capital needed | +12 tháng timeline |
| **Option B: Strategic investor** | Vingroup, Viettel, Thaco (corporate VC) | +3-5M | Cooperation/JV | +6 tháng negotiation |
| **Option C: Bootstrap + vay** | - Tăng vay từ 6M → 8M USD<br/>- Cash flow reinvestment năm 2-3 | +2-3M | Bank credit limit | +6 tháng, higher debt |
...
| **Quality System** | ISO 9001/14001/45001, QA/QC 3 lớp | 0,6M USD | 2025-2027 | Yield 99,5%, RMA <0,1% (stretch goal), Base Case <0,3% |
...
```mermaid
graph TB
    A[MekongOS Platform] --> B[Device Management]
    A --> C[Data Pipeline]
    A --> D[Analytics & AI]
    ...
    C --> C1[Ingestion MQTT/HTTP]
    C --> C2[Time-series DB]
    C --> C3[Data Lake]
    C --> C4[Stream Processing]
    
    D --> D1[Dashboard]
    D --> D2[Anomaly Detection]
    D --> D3[Predictive Maintenance]
    D --> D4[Optimization]
    
    E --> E1[Fleet Management]
    E --> E2[Energy Management]
    E --> E3[Asset Tracking]
    E --> E4[Custom Apps]
```
...
gantt
    ...
    6G-enabled Smart Devices     :         g4_3, 2042, 2045
    AR/VR Remote Robotics Control :        g4_4, 2043, 2045
```
...
```mermaid
flowchart TB
    A[R&D & Design] --> B[Component Sourcing]
    B --> C[Manufacturing]
    C --> D[QA/QC Testing]
    D --> E[Packaging]
    E --> F[Logistics & Delivery]
    F --> G[Installation & Training]
    G --> H[After-sales Support]
    
    A --> A1[PCB Design]
    A --> A2[Firmware Development]
    A --> A3[Mechanical Design]
    
    C --> C1[SMT Assembly]
    C --> C2[Mechanical Assembly]
    C --> C3[Final Integration]
...
flowchart LR
    ...
    B -.->|Reject| B1[Supplier Return]
    C -.->|Rework| C1[Rework Station]
    D -.->|Reject| D1[Scrap/Analysis]
```
...
```mermaid
graph TB
    A[Open Innovation] --> B[Internal R&D]
    A --> C[External Partnerships]
    A --> D[Customer Co-creation]
    A --> E[Acquisition/Licensing]
    ...
    F --> I[Fail Fast]
    F --> J[Continuous Delivery]
    
    G --> G1[2-week sprints]
    H --> H1[3D printing<br/>Rapid PCB]
    I --> I1[MVP testing<br/>Pivot quickly]
    J --> J1[OTA updates<br/>CI/CD]
```
...
|---|---:|---:|---:|---:|---:|---|
| **GĐ1 (2025-2029)** | 18,36 (trung bình) | 2,15 | **5,0%** | 36 | 60K | Technology acquisition |
| **GĐ2 (2030-2035)** | 55,08 (trung bình) | 3,30 | **3,0%** | 60 | 55K | Localization & scale |
| **GĐ3 (2036-2075)** | 46,27 (trung bình) | 2,31 | **2,5%** | 100 | 23K | Mature products, maintenance |
...
| 7 | Phí bản quyền, IP | 0,20 | 0,15 | 0,12 | Royalty KUKA 3% doanh thu, patents filing (5K USD/patent), trademark |
| **Tổng** | | **2,15** | **3,28** | **4,95** | |
...
|---|---:|---:|---:|---:|---|
| **GĐ1 (2025-2029)** | 2,15 | 6,43 | **33,5%** | ≥10% | ✅ Vượt chuẩn gấp 3 lần |
| **GĐ2 (2030-2035)** | 3,28 | 19,28 | **17,0%** | ≥10% | ✅ Vượt chuẩn gần 2 lần |
| **GĐ3 (2036-2075)** | 4,95 | 16,20 | **30,6%** | ≥10% | ✅ Vượt chuẩn gấp 3 lần |

*Công thức: VA = Doanh thu - COGS - Opex (excluding R&D) = Gross Profit × 0,7, theo chuẩn IFRS*

**Kết luận:** Dự án Mekong Technology **duy trì tỷ lệ R&D/VA 17-34%** (trung bình 27%) trong suốt 50 năm hoạt động, **vượt xa tiêu chí 10%** của doanh nghiệp công nghệ cao theo [NĐ 76/2018/NĐ-CP – Điều 4, Khoản 2 (Tiêu chí doanh nghiệp CNCT: R&D/VA ≥10%)] (Chính phủ, 2018). Điều này cho thấy cam kết mạnh mẽ về R&D và đổi mới công nghệ.
...
|---|---:|---:|---|---:|---:|
| **Fresh graduates** | 15 | 40% | ĐH Bách Khoa HCM/HN, Cần Thơ, Đà Nẵng<br/>Training intensiveNhững người muốn đổi career | 30K USD/năm | 450.000 |
| **Experienced (Headhunt)** | 16 | 45% | Từ FPT, Viettel, Samsung, Intel, Bosch<br/>Mid-level (3-8 years) | 80K USD/năm | 1.280.000 |
| **Expat/Consultant** | 5 | 15% | Chuyên gia từ KUKA, Tuya, freelancer<br/>Short-term (6-12 months) | 120K USD/năm | 600.000 |
...
|---|---|---|---|---:|
| **Onboarding** | 3 tháng | Product training (1 tháng)<br/>Clean Room RE process (1 tháng)<br/>QA/QC & Safety (2 tuần)<br/>Company culture & Tools (2 tuần) | CTO + HR | 5.000 |
| **Technical upskilling** | 2 tuần/năm | Conferences (IEEE, NeurIPS)<br/>Online courses (Coursera, Udemy)<br/>Workshops & hands-on | Lab Leads | 3.000 |
...
| **ESOP (Employee Stock Option Plan)** | 5-10% equity pool, vesting 4 năm<br/>Cliff 1 năm (25%), monthly sau đó | Retain >85% sau 3 năm |
| **Career path rõ ràng** | Engineer → Senior (2-3 năm) → Lead (3-4 năm) → Director (5+ năm)<br/>Technical track // Management track | Motivation cao |
| **Work-life balance** | Flexible hours (9-6 core, arrival 8-10)<br/>Remote 2 ngày/tuần<br/>20 days PTO + 10 sick days | Satisfaction >80% |
| **Learning culture** | Conference budget 5K USD/person/năm<br/>Book allowance 500 USD/year<br/>Internal tech talks weekly | Innovation |
| **Modern equipment** | MacBook Pro/Dell XPS, 2x monitor, ergonomic chair<br/>Budget: 3K USD/person | Productivity |

**Phương án dự phòng (nếu không tuyển đủ 36 người):**

| Kịch bản | Giải pháp | Chi phí | Timeline impact |
|---|---|---|---|
| **Thiếu 30% (11 người)** | Outsource 30% R&D cho công ty Ấn Độ/Đông Âu (Luxoft, EPAM)<br/>Chi phí: 40K USD/person/năm (rẻ hơn 40%) | Save 220K USD | +3 tháng (coordination overhead) |
| **Thiếu 50% (18 người)** | Partnership với 2 trường ĐH (Bách Khoa, TDT): Part-time researchers<br/>Graduate students (Master/PhD): 15K USD/year | Save 810K USD | +6 tháng (quality risk) |
...
```mermaid
flowchart TB
    CEO[CEO] --> BOD[Board of Directors]
    CEO --> CTO[CTO]
    CEO --> CFO[CFO]
    CEO --> COO[COO]
    CEO --> CMO[CMO]
    ...
    CTO --> RD[R&D: 36]
    CFO --> FIN[Finance & Accounting: 8]
    COO --> PROD[Production: 120]
    COO --> QA[QA/QC: 15]
    COO --> SCM[Supply Chain: 10]
    CMO --> MKT[Marketing: 5]
    CMO --> SALES[Sales: 20]
    CPO --> PM[Product Management: 5]
    CPO --> DESIGN[Design: 3]
    
    CEO --> HR[HR & Admin: 10]
    CEO --> LEGAL[Legal & Compliance: 3]
    CEO --> IT[IT Support: 5]
```
