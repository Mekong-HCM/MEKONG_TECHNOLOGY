        ...
        <SectionLayout id="product-iot">
            <div ref={ref}>
                <motion.div initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
                    ...
                    <p className="section-subtitle">Industrial IoT Gateway Made in Vietnam — ARM Cortex, AI tai bien, Multi-protocol, 5G-ready</p>
                </motion.div>
                        ...
                        <table className="w-full text-xs">
                            <thead>
                                <tr className="border-b border-white/10">
                                    <th className="text-left py-2 px-3 text-gray-400 font-medium w-1/3">Thong so</th>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                    <motion.div initial={{ opacity: 0, x: -20 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ delay: 0.25, duration: 0.5 }}>
                        <GlassCard className="p-5 h-full">
                            <h4 className="text-sm font-bold text-white mb-3 flex items-center gap-2"><Cpu size={16} className="text-neon-cyan" /> Dac diem noi bat</h4>
                            <ul className="space-y-2">
                                {[
                                    { icon: Wifi, text: 'Multi-protocol: MQTT, OPC UA, Modbus, EtherCAT, PROFINET' },
                                    { icon: Zap, text: 'AI Edge: Vision inference, predictive maintenance tai bien' },
                                    { icon: Shield, text: 'Bao mat: TLS 1.3, TPM 2.0, Secure Boot, encrypted OTA' },
                                    { icon: Cpu, text: 'ARM Cortex + NPU 6 TOPS — xu ly AI khong can cloud' },
                                ].map((item, i) => (
                    ...
                    <motion.div initial={{ opacity: 0, x: 20 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ delay: 0.3, duration: 0.5 }}>
                        <GlassCard className="p-5 h-full">
                            <h4 className="text-sm font-bold text-white mb-3">Cong suat san xuat</h4>
                            <div className="grid grid-cols-2 gap-3">
                                {[
                                    { label: 'Tong/nam', value: gatewayCapacity.total, color: 'text-neon-cyan' },
                            ...
                            <div className="mt-3 text-[10px] text-gray-500 flex items-center gap-1">
                                <Check size={10} className="text-green-400" /> Thay the Siemens SIMATIC, Schneider EcoStruxure — gia re hon 20-30%
                            </div>
                        </GlassCard>
                    </motion.div>
                </div>
